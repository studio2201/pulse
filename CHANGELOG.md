...
### Changed
- **Containerized Admin Console integration**: Named the admin tool after the application (`pulse`) and copied it to the container's system path `/usr/local/bin/pulse`. The TUI can now be launched by simply executing `pulse tui` (or `pulse`) inside the container shell.
